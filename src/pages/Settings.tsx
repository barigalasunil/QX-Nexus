...
import { Plus, Trash2, Shield, UserX, UserCheck, Key, Settings as SettingsIcon, X, HardDrive, Calendar, Bell, Save, CalendarDays } from 'lucide-react';
...
export function Settings({ currentUser, appState, setAppState, showToast, theme, readOnly = false, onUpdateCurrentUser }: SettingsProps) {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    else if (!userForm.email) nextErrors.email = 'Email address is required.';
    ...
    // Supabase path: Edge Function generates password + creates auth user.
    // Local path: generate + hash password here.
    ...
    // STEP 2 — Handle password (local path only; Supabase Edge Function generates it)
    let localPlainPassword: string | null = null;
    if (!useSupabase) {
      localPlainPassword = generateStrongPassword();
      const hashedPassword = await hashPassword(localPlainPassword);
      newUser.password = hashedPassword;
      await UserService.createUser(currentUser, newUser, localPlainPassword);
    } else {
      await UserService.createUser(currentUser, newUser);
    }
    ...
    // Supabase: retrieve the password generated by the Edge Function.
    // Local: use the plain password generated in step 2.
    if (useSupabase) {
      const serverPassword = await UserService.popGeneratedPassword();
      setGeneratedEmail(userForm.email);
      setGeneratedPassword(serverPassword || '');
    } else {
      setGeneratedEmail(userForm.email);
      setGeneratedPassword(localPlainPassword || '');
    }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {allowedRoles.length > 0 && canEditSettings && <div style={commonStyles.card(theme)}>
            ...
            <form noValidate onSubmit={handleAddUser} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
              ...
              <Field label="Email Address" type="email" placeholder="user@company.com" value={userForm.email} onChange={(v) => updateUserForm('email', v)} error={userErrors.email} required theme={theme} />
              <Field
                label="Role"
                type="select"
                value={userForm.role}
